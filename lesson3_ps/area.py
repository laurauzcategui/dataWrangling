...
You have to change the function fix_areac. You can use extra functions if you like, but changes to process_file
...
def fix_area(area):
    ...
    if str(area) == "NULL":
        area = None
    elif re.search('^{.*', str(area)):
        aux_area = re.split('[|]', area.strip('{').strip('}'))
        area = float(filter(lambda x : len(x) == max(map(len, aux_area)),aux_area)[0])
    else:
        area = float(area)
    return area
